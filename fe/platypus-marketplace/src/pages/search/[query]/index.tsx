...
const Search = ({ products, paramQuery, pagination_info }: ISearchProps) => {
  const { setProducts, getFilterSortSearch, setPaginationInfo } =
    useProducts.getState();
  const { setQ, setSortBy, setSort } = useFilter.getState();
...
export const getServerSideProps: GetServerSideProps = async (params) => {
  ...
  const response = await fetch(
    `${process.env.BASE_API_URL}/products?q=${paramQuery}`,
  );
