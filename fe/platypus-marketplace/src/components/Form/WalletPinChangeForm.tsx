...
interface IWalletPinChangeForm {
  verifiedToken: string;
}

const WalletPinChangeForm = ({ verifiedToken }: IWalletPinChangeForm) => {
  ...
  const handleSubmitPin = async () => {
    showLoadingSm();
    const { error, message, code } = await service.postChangeWalletPin(
      { pin },
      verifiedToken,
    );
    ...
    if (error && code === 500) {
      ...
      hideLoadingSm();
      toast({
        title: "Invalid/Time out token",
        variant: "destructive",
      });
      router.push("/user/platypay");
      ...
      hideLoadingSm();
      toast({ title: "Change wallet pin success", variant: "success" });
      router.push("/user/platypay");
    }
  };
...
export default WalletPinChangeForm;
